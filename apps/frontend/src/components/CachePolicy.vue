...
import SegmentStats from './SegmentStats.vue'
...
<template>
  <section
    class="p-4 leading-5 bg-white border-solid rounded-lg shadow-xl b-1 shadow-black/5 ring-1 ring-slate-700/10"
  >
    ...
    <div class="flex w-full gap-10 my-5">
      <template v-for="(value, key, index) in lastSnapshot">
        <div v-if="key !== 'timestamp'" class="flex-1 space-y-2" :key="key">
          <p
            class=""
            :class="`block text-xs uppercase border-b-2 text-slate-400 border-[${colors[index]}]`"
          >
            {{ key.toUpperCase() }}
          </p>
    ...
    <Disclosure v-slot="{ open }">
      ...
      >
        <p class="text-xs">Segments ({{ props.policy.segments.length }})</p>
